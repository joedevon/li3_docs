<div class="nav"><nav>
	<?php if ($object['children']) { ?>
		<h2>
			<?=$t('{:scope} contents', array(
				'scope' => isset($object['library']) ? $object['library'] : 'file'
			)); ?>
		</h2>
		<ul class="children">
			...
			?>
			<li class="<?=is_array($type) ? 'namespace' : $type; ?>">
			...
			<?php if (is_array($type) && count($type) >= 1) { ?>
				<ul>
					<?php foreach ($type as $name2 => $type2) { ?>
						...
						?>
						<li class="<?=is_array($type2)? 'namespace' : $type2; ?>">
							<?=$this->html->link($name2, $url); ?>
						</li>
